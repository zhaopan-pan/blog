...
<div style="display: flex" class='article-excerpt'>
...
最近在重写[博客主题](https://github.com/zhaopan-pan/vuepress-theme-zp)，基于[vuepress2](https://v2.vuepress.vuejs.org/)的默认主题([theme-default](https://github.com/vuepress/vuepress-next/tree/main/ecosystem/theme-default))，原主题是知识文档库的风格，不满足[blog](https:zhaopanpan.com)的功能，所以就开始了魔改，在魔改的过程中少不了要研究源码，发现了一些'问题'的代码

## 问题
### 1、scss文件和组件之间各自维护相同变量
...
### 2、webpack下启动报错
然后就提了pr，维护者很快就进行了review，当我以为事情快要结束时，项目的[check](https://github.com/vuepress/vuepress-next/actions/runs/4827419222/jobs/8600551762)阶段却报错了😭，奇了怪了，明明本地启动正常的。报错内容如下
```sh
- Cleaning temp
...
## 总结
在写blog主题的过程中,对源代码中不够优雅的变量使用做了一个小小优化，不过却遇到了一些坑，通过查阅源码一步一步进行了解决。后面也对css变量的使用做了进一步优化，封装了公共方法，有兴趣的平移(不是好热的)查看[pr](https://github.com/vuepress/vuepress-next/pull/1322)。
